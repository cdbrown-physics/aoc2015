...
def mover(d, location, spots):
        if d == '^':
            location[1] += 1
            if str(location) in spots:
                spots[str(location)] += 1
            else:
                spots[str(location)] = 1
        elif d == 'v':
            location[1] -= 1
...
if __name__ == "__main__":
    ...
    spots = {str(location): 1}
    for d in directions:
        location, spots = mover(d, location, spots)
    
    print(len(spots.keys()))
    
    
    realLocation = [0,0]
    roboLocation = [0,0]
    spots = {str(realLocation): 1}
    roboTurn = True
    
    for d in directions:
        if roboTurn:
            roboLocation, spots = mover(d,roboLocation, spots)
        elif not roboTurn:
            realLocation, spots = mover(d, realLocation, spots)
